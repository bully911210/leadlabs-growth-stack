...
const ALLOWED_ORIGIN = "https://leadlabs.co.za";
...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    const confirmationEmailResponse = await resend.emails.send({
      ...
      html: `
        <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 600px; margin: 0 auto;">
          <h1 style="color: #6C4EFF; border-bottom: 3px solid #6C4EFF; padding-bottom: 16px;">Thanks for reaching out, ${safeName}!</h1>

          <p style="font-size: 16px; line-height: 1.6; color: #333;">
            We've received your inquiry about <strong>${safeService}</strong> and Franz Badenhorst will personally review it.
          </p>
          ...
          <p style="color: #666; font-size: 14px; margin-top: 40px;">
            ...
            Managing Director, SIG Solutions<br>
    ...
    console.error("Error in send-contact-email function:", errMessage);
    return new Response(
      JSON.stringify({
        error: errMessage,
        success: false
